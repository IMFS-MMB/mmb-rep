...
var cnk,ynk,ink,knk,hnk,lnk,psink,pnnk,pdnk,dpnk,dynk,enk,dnk,Rsnk,Rdnk,Rrnk,lbnk,Util,Welf,kpnk; 
...
disp([kpss css hss (log(css)-thh*hss^(1+chi)/(1+chi))/(1-nbeta)])
...
model;     
    ...
    // Banking aspects 
    exp(enk)    = exp(lnk)*(1-tau)*((1-phiss)*gammass*(exp(etar)-1)+exp(psil)*((1-phiss)*gammass*exp(etar)-phiss))/(phiss*(1+gammass)-gammass*tau*(1-phiss)*(exp(etar)-1)); 
    ...
    Rrnk        = Rsnk+etar; 
    exp(kpnk)   = (1-tau)*((1-phiss)*gammass*(exp(etar)-1)+exp(psil)*((1-phiss)*gammass*exp(etar)-phiss))/(phiss*(1+gammass)-gammass*tau*(1-phiss)*(exp(etar)-1)); 
...
%stoch_simul(order=2,irf=20,nograph) knk; 
%stoch_simul(order=1,irf=20,nograph,periods=100000) kap;
%if year==2007
%disp(sum(and(kap>=0.1,kap<=0.14))/length(kap))
%else
%disp(sum(and(kap>=0.08,kap<=0.16))/length(kap))
%end
%kap=kpss*exp(kpnk); 

Wss=(oo_.steady_state(1)-thh*exp((1+chi)*oo_.steady_state(5))/(1+chi))/(1-nbeta);
tmp=sqrt(diag(oo_.var));
Wflux=-(1+chi)*thh*exp(oo_.steady_state(5)*(1+chi))*tmp(5)*tmp(5)/(2*(1-nbeta)); 
%save welf_m2_2012 Wss Wflux
//save(sprintf('results/welf_m2_%d',year),'Wss','Wflux')

//save(sprintf('results/results_m2_%d',year),'M_','oo_')