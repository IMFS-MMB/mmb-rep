...
/*****************************************************************
  This version of print_functions() is for use with Matlab 3.5.
  Slightly different versions of the function, for use with
  Matlab 4.0 and SPlus, are in the files print_functionsc and
  print_functions_splus.c, respectively.
******************************************************************/
...
******************************************************************/
  ...
  printf("  *eqname = (char **) calloc(%d, sizeof(char*));\n", 
	  m->neq);
  printf("  *param = (char **) calloc(%d, sizeof(char*));\n", 
	  m->ncoeffs);
  printf("  *endog = (char **) calloc(%d, sizeof(char*));\n", 
	  m->neq);
  printf("  *delay = (int *) calloc(%d, sizeof(int));\n", m->neq);
  printf("  *vtype = (int *) calloc(%d, sizeof(int));\n", m->neq);
  printf("  *eqtype = (int *) calloc(%d, sizeof(int));\n", m->neq);
  printf("\n");
...
******************************************************************/
  ...
  printf("  *g = (double *) calloc(%d, sizeof(double));\n", 
	  (m->nlag + 1) * m->neq * m->neq);
  printf("  *h = (double *) calloc(%d, sizeof(double));\n", 
