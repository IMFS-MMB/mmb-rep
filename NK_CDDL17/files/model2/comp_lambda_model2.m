function [errcheck,lmss_vals]=comp_lambda_model2(xx)
...
lmss_mat(10,12) = -(exp(lnk_SS)*(exp(psil_SS)*(phiss + gammass*exp(etar_SS)*(phiss - 1)) + gammass*(exp(etar_SS) - 1)*(phiss - 1))*(tau - 1))/(phiss*(gammass + 1) + gammass*tau*(exp(etar_SS) - 1)*(phiss - 1)); 
