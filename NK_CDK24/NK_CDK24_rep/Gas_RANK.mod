...
C_gap      = 100*log(C_u/C_c);% unconstrained over constrained, 100*log diff, approx percent difference
C_gap_flex = 100*log(C_u_flex/C_c_flex); 
...
% objective 1
    planner_objective 
    ( ((1-omega)*
    (CES_u)^(1-siggma)-1)/(1-siggma) - chi_u*( (1-omega)*(dispw*N_u)^(1+varphi))/(1+varphi) ) 
    +    ( (omega*(CES_c)^(1-siggma)-1)/(1-siggma) - chi_c*( omega*(dispw*N_c)^(1+varphi))/(1+varphi) )
    ;   
