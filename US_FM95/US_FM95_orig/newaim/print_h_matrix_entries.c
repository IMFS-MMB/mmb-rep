...
/*******************************************************************
	This version of print_h_matrix_entries.c, 
   called print_h_matrix_entries_m.c, is for use with
...
void print_h_matrix_entries(n, m, eqnum, side, fp)
     struct node *n; struct model *m; int eqnum; enum sidetype side; FILE *fp;
{
  ...
  case VARIABLE: 
	...
	index = ((n->info.var.period + m->nlag) * m->neq + 
	  ...
	  * m->neq + eqnum;
	fprintf(fp, "  h(%d) = h(%d)", index+1, index+1);
	if (side == LEFT_SIDE)
	  fprintf(fp, " + 1;\n");
	else
	  fprintf(fp, " - 1;\n");
    ...
    break;
  case SUM:
    print_h_matrix_entries(n->info.sum.summand1, m, eqnum, side, fp);
    print_h_matrix_entries(n->info.sum.summand2, m, eqnum, side, fp);
    break;
  case PRODUCT:
