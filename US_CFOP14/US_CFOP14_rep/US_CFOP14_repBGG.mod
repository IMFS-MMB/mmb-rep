...
// Non SD parameters 
// ------------------------------------------------------------------------
alpha= 0.17;     // share of capital in the prod. function
delta= 0.025;    // capital depreciation rate
iotap= 0.13;     // price indexation (=0 is static indexation, =1 is dynamic)
iotaw= 0.12;     // wages indexation
gamma100= 0.49;  // steady state growth rate of technology (multiplied by 100)
h= 0.85;         // habit formation 
lambdapss= 0.20; // steady state net price markup
lambdawss= 0.15; // steady state net wage markup
Lss= exp(0.40);       // steady state for log hours
pss100= 1.0063;    // steady state quarterly net inflation (multiplied by 100)
Fbeta= 0.14;     // 100 * ( inv(discount factor) - 1 )
gss = 1/(1-0.22);       // steady state government spending to GDP ratio
niu= 4.64;       // inverse Frisch elasticity
xip= 0.79;       // price stickiness
xiw= 0.87;       // wage stickiness
chi= 4.68;       // elasticity of the capital utilization cost function
Sadj= 1.09;      // investment adjustment cost
fp= 1.35;        // reaction to inflation
fy= 0.04;        // reaction to output gap
fdy= 0.27;       // reaction to output gap growth
rhoR= 0.83;
rhoz= 0.24;
rhog= 0.99;
rhomiu= 0.43;
