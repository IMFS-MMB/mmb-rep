...
% Parameter values make the model bimonthly and reflect the estimates based on the      %
% 	modern (89-09) sample and the sector-specific (rather than GDP) deflator.	%
%	Also, re-estimated to match BLS price change frequency and no price indexation.	%
%	No price-markup shocks.  (parameters estimated so that price markup shocks	%
%	   were measurement error, but now, we set std error or meas error to 0)	%
%											%
% BAM 08/02/11, this version 8/11/11							%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
// Estimated parameters (from posterior mode of usmodel_bimonth_est_err.mod, for estimated curvp)
...
ctrend = 0.240;    
calfa=.234;
curvp = 54.104; //Estimated Kimball Kimk for final goods;
...
stderr 0.040;
var epinf;
stderr 0;  //No price mark-up shocks;
var ew;
stderr 0.483;
end;
